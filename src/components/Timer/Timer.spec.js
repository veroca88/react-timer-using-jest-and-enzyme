...
describe("mounted Timer", () => {
  ...
  it("invokes startTimer when the start button is clicked", () => {
    const jestSpy = jest.spyOn(container.instance(), "startTimer");
  ...
  it("invokes stopTimer when the stop button is clicked", () => {
    const jestSpy = jest.spyOn(container.instance(), "stopTimer");
    container.instance().forceUpdate();
    expect(spy).toHaveBeenCalledTimes(0);
  ...
  it("invokes resetTimer when the reset button is clicked", () => {
    const jestSpy = jest.spyOn(container.instance(), "resetTimer");
    container.instance().forceUpdate();
    expect(spy).toHaveBeenCalledTimes(0);
